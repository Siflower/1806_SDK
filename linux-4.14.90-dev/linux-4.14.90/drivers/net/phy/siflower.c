...
static int sf_genphy_read_mmd_unsupported(struct phy_device *phdev, int devad, u16 regnum)
{
	return -EOPNOTSUPP;
}
static int sf_genphy_write_mmd_unsupported(struct phy_device *phdev, int devnum,
				u16 regnum, u16 val)
{
	return -EOPNOTSUPP;
}
#else
#define sf_genphy_read_mmd_unsupported       genphy_read_mmd_unsupported
#define sf_genphy_write_mmd_unsupported      genphy_write_mmd_unsupported
#endif
...
#endif
static void sf12xx_c22_reg_op_help(void)
{
    pr_notice("\nUsage:\n"
...
static ssize_t sf12xx_c22_reg_op(struct file *file,
                    ...
                    loff_t *data)
{
    ...
    if (buf[0] == 'w') {
        ...
        pr_notice("\n read back phy=%d, reg=0x%x, val=0x%x\n",
            phydev->mdio.addr, reg, val);
    } else if (buf[0] == 'r') {
        if (sscanf(buf, "r %x", &reg) == -1)
            return -EFAULT;
        val = phy_read(phydev, reg);
        pr_notice("\n read phy=%d, reg=0x%x, val=0x%x\n",
            phydev->mdio.addr, reg, val);
    } else {
        sf12xx_c22_reg_op_help();
    }
    return count;
}
static const struct file_operations sf12xx_c22_reg_fops = {
    .owner = THIS_MODULE,
    .open = simple_open,
    .write = sf12xx_c22_reg_op,
    .llseek = noop_llseek,
};
static void sf12xx_ext_reg_op_help(void)
{
    pr_notice("\nUsage:\n"
            "[debugfs] = /sys/kernel/debug/mdio-bus\':[phy_addr]\n"
            "Read:\n"
...
static ssize_t sf12xx_ext_reg_op(struct file *file,
                    const char __user *buffer, size_t count,
                    loff_t *data)
{
    struct phy_device *phydev = file->private_data;
    ...
    if (buf[0] == 'w') {
        ...
        pr_notice("\nwrite ext reg=0x%x, val=0x%x\n",
            reg, val);
        ret = siflower_phy_ext_write(phydev, reg, val);
        if (ret < 0) {
            pr_notice("\nsiflower_phy_ext_write fail\n");
            count =  -EIO;
            goto err;
        }
        val = siflower_phy_ext_read(phydev, reg);
        pr_notice("\nread back ext reg=0x%x, val=0x%x\n",
            reg, val);
    } else if (buf[0] == 'r') {
        if (sscanf(buf, "r %x", &reg) == -1) {
            count =  -EFAULT;
        ...
        sf12xx_ext_reg_op_help();
    }
err:
    return count;
}
static const struct file_operations sf12xx_ext_reg_fops = {
    .owner = THIS_MODULE,
    .open = simple_open,
    .write = sf12xx_ext_reg_op,
    .llseek = noop_llseek,
};
static int sf12xx_debugfs_init(struct phy_device *phydev)
{
    int ret = 0;
    struct sf12xx_priv *priv = phydev->priv;
    struct dentry *dir = priv->debugfs_root;
    dev_info(phydev_dev(phydev), "%s: start\n", __func__);
    ...
    debugfs_create_file("c22_reg_op", S_IFREG | 0200,
            dir, phydev,
            &sf12xx_c22_reg_fops);
    debugfs_create_file("ext_reg_op", S_IFREG | 0200,
            dir, phydev,
            &sf12xx_ext_reg_fops);
...
static int sf1xxx_probe(struct phy_device *phydev)
{
        ...
        struct sf12xx_priv *priv;
        int ret;
        priv = devm_kzalloc(dev, sizeof(*priv), GFP_KERNEL);
    if (!priv)
                return -ENOMEM;
        phydev->priv = priv;
        ret = sf12xx_debugfs_init(phydev);
    if (ret < 0) {
        phydev_err(phydev, "sf12xx_debugfs_init fail. (ret=%d)\n", ret);
    }
        return ret;
}
static void sf12xx_debugfs_remove(struct phy_device *phydev)
{
    struct sf12xx_priv *priv = phydev->priv;
    debugfs_remove_recursive(priv->debugfs_root);
    priv->debugfs_root = NULL;
}
static void sf12xx_remove(struct phy_device *phydev)
{
    struct sf12xx_priv *priv = phydev->priv;
    if (priv) {
        sf12xx_debugfs_remove(phydev);
        kfree(priv);
        priv = NULL;
    }
}
...
static struct phy_driver sf_phy_drivers[] = {
	{
		...
		.write_mmd          = sf_genphy_write_mmd_unsupported,
		.read_mmd           = sf_genphy_read_mmd_unsupported,
#endif
