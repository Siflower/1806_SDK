				      ...
				      const struct nand_page_io_req *req)
{
	...
	if (req->ooblen) {
		if (req->mode == MTD_OPS_SF_FIXED || req->mode == MTD_OPS_AUTO_OOB)
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	if (ops->mode != MTD_OPS_SF_FIXED && ops->mode != MTD_OPS_RAW && spinand->eccinfo.ooblayout)
		enable_ecc = true;
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	if (ops->mode != MTD_OPS_SF_FIXED && ops->mode != MTD_OPS_RAW && mtd->ooblayout)
		enable_ecc = true;
