...
static void split_rootfs_data(struct mtd_info *master, struct mtd_part *part)
{
	...
	if (IS_ENABLED(CONFIG_MTD_SPLIT_SQUASHFS_ROOT))
	...
	mtd_add_partition(master, ROOTFS_SPLIT_NAME, split_offset,
			    split_size);
}
...
static void split_uimage(struct mtd_info *master, struct mtd_part *part)
{
	...
	if (IS_ENABLED(CONFIG_MTD_SPLIT_UIMAGE_FW))
	...
	mtd_add_partition(master, "rootfs", part->offset + len,
			    part->mtd.size - len);
}
...
static void split_firmware(struct mtd_info *master, struct mtd_part *part)
{
	...
	if (ret > 0)
		return;
	if (IS_ENABLED(CONFIG_MTD_UIMAGE_SPLIT))
...
static void mtd_partition_split(struct mtd_info *master, struct mtd_part *part)
{
	...
	if (!strcmp(part->mtd.name, "rootfs")) {
		...
		if (num <= 0 && IS_ENABLED(CONFIG_MTD_ROOTFS_SPLIT))
