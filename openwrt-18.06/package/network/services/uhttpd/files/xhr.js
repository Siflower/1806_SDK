...
XHR = function()
{
    ...
    this.get_with_host = function(host,url,data,callback)
    {
        this.reinit();
    ...
    this.post_siflower = function(url,data,callback)
    {
        this.reinit();
    ...
    this.upload = function(url, formdata, callback, progresscb)
    {
        this.reinit();
        var xhr  = this._xmlHttp;
        ...
        if (Object.prototype.toString.call(formdata) === '[object FormData]') {
            ...
            if (progresscb != null){
                xhr.upload.onprogress = function (evt) {
                    progresscb(Math.round(evt.loaded / evt.total * 100) + "%");
                }
            }
            xhr.open('POST', url, true);
...
XHR.get_with_host = function(host, url, data, callback)
{
    (new XHR()).get_with_host(host, url, data, callback);
}


XHR.post = function(url, data, callback)
{
    (new XHR()).post_siflower(url, data, callback);
}

//progresscb 为进度条的回调方法，暂时还没有实现
XHR.upload = function (url, formdata, callback, progresscb) {
    (new XHR()).upload(url, formdata, callback, progresscb);
}
