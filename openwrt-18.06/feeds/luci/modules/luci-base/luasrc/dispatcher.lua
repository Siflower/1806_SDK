...
function authen_json(validator, allowed_users)
    --    local user = luci.http.formvalue("username")
	local user = http.getenv("HTTP_AUTH_USER")
	local passwd = http.getenv("HTTP_AUTH_PASS")
    ...
    if(userid_local and userid_local ~= "" and validator(user, passwd)) then
        return user,nil,userid_local,passwd
    end

	local pass = arg_list_table["luci_password"]
    local result = {}
    local code = 1011
	if pass then
		user = "root"
        if validator(user, pass) then
            return user,nil,userid_local,pass
        else
            code = 1005
        end
	end
...
function dispatch(request)
	...
	--if not conf.main or  conf.main == nil then
		--nixio.syslog("crit","rpcd seems to be corrupt, restart rpcd here")
	...
	if track.sysauth and not ctx.authsession then
		...
		if type(authen) == "string" and authen ~= "htmlauth" and authen ~= "jsonauth" then
			error500("Unsupported authenticator %q configured" % authen)
			return
		end

		if type(track.sysauth) == "table" then
			if #track.sysauth == 2 then
				default_user = "root"
			else
				default_user = nil
			end
			allowed_users = track.sysauth
		else
		...
		if not (sid and sdat) and authen == "jsonauth" then
			local user, sess ,userid ,passwd  = authen_json(sys.user.checkpasswd, allowed_users)

			nixio.syslog("crit","httpdispatch--authen_json user "..tostring(user)..tostring(passwd))
			if not user or not util.contains(allowed_users, user) then
				return
			else
				cur_userid = userid
				if not sess then
					local sda = util.ubus("session", "login", {
						username = user,
						password = passwd,
						timeout  = tonumber(luci.config.sauth.sessiontime)
					})
					if sda == nil then
						--nixio.syslog("crit","rpcd debug here, because session login fail")
						--luci.util.exec("/etc/init.d/rpcd restart")
						--os.execute("sleep "..2)
						sda = util.ubus("session", "login", {
							username = user,
							password = passwd,
							timeout  = tonumber(luci.config.sauth.sessiontime)
						})
					...
					if sda then
						util.ubus("session", "set", {
							ubus_rpc_session = sda.ubus_rpc_session,
							values = { 
		...
		if not (sid and sdat) and (type(authen) == "table" or authen == "htmlauth") then
			local user = http.getenv("HTTP_AUTH_USER")
