	...
	execute: function () {
		...
		$('#wds_en').click(function () {
			if ($('#wds_en').prop('class') == 'switch_on') {
				...
				let ip = L.uci.get('network' , 'lan', 'oip');
								...
								$('<div />')
									...
									.append(
										$('<div />')
											...
											.text(L.tr("To switch from routing mode to WDS mode, please ensure that the network cable is unplugged. If your Wi Fi is turned off, clicking 'Continue' will automatically turn on Wi Fi. At this point, the connection between your router and browser will be disconnected. Please reconnect later.")),
									),
							)
							.append(
								$('<div />')
			...
			self.wifi_status(L.globals.ifname).then(function (status) {
				...
				if(status == 'ok'){
					if(en_count == 0) {
						...
						clearInterval(ifwi1);
						$('body').find('.loading').remove();
						$('body').append(loadbox(L.tr("Connect success, ready to start wds. Please refresh page two minutes later. If you cannot return the now page means the wds connect success.After successful relay, please enter the IP address after relay to log in.")));
