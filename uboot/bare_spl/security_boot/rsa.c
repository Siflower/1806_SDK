...
#define calloc(m , n)		m_calloc(m ,n)
#define mbedtls_free   free
#endif

extern void* m_calloc(size_t m , size_t n);
/*
...
#include "sha1.h"
/*
* the image contains: uboot, sign info, pubkey
*|----uboot----|---sign-----|----pubkey----|
* the sign info and pubkey are both 128bytes
*/
int rsa_verify_image(void *image, int len, void *key)
{

	unsigned int e = 0x10001; //default public exponent
    mbedtls_rsa_context rsa;
    unsigned char sha1sum[20];
	unsigned char *sign;

    mbedtls_sha1(image, len - 256, sha1sum);
    mbedtls_rsa_init( &rsa, MBEDTLS_RSA_PKCS_V15, 0 );
    rsa.len = RSA_KEY_LEN;

	memcpy(key , image + len - 128, RSA_KEY_LEN);

	sign = (unsigned char *)(image + len - 256);

	// read pubkey
	//mbedtls_mpi_read_binary(&rsa.N, (unsigned char *)(image + len - 128), 128);
	mbedtls_mpi_lset(&rsa.E, e);
	rsa.N.s = 1;
	rsa.N.p = (mbedtls_mpi_uint *)(key);
	rsa.N.n = RSA_KEY_LEN / sizeof(mbedtls_mpi_uint);

#if 0
	rsa.E.s = 1;
	rsa.E.p = (mbedtls_mpi_uint *)&e;
	rsa.E.n = 1;
#endif
