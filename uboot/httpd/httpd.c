...
#define UIP_HTTPD_FILE_NAME_LEN		32
...
char uip_httpd_file_name[UIP_HTTPD_FILE_NAME_LEN] = {0};
...
static int httpd_findandstore_firstchunk(void){
	...
	if(start){
		...
		if (NULL != (end = strstr(start, "name=\"firmware\"")))
		{
			printf("Upgrade type: firmware\n");
			webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE;
		}
		else if (NULL != (end = strstr(start, "name=\"uboot\"")))
		{
...
#ifndef CONFIG_SFA18_UBOOT_LITE
		else if (NULL != (end = strstr(start, "name=\"art\"")))
		{
...
#endif
		else if (NULL != (end = strstr(start, "name=\"test\"")))
		{
			/*
			 * You must ensure that the
			 * buffer(uip_httpd_file_name,) does not overflow
			 *
			 * */
			if (strstr(start, "u-boot") != NULL) {
				printf("Upgrade type: TEST Uboot\n");
				strcpy(uip_httpd_file_name, "uboot");
			} else if (strstr(start, "uboot") != NULL) {
				printf("Upgrade type: TEST Uboot with spl\n");
				strcpy(uip_httpd_file_name, "spl_uboot");
			} else if (strstr(start, "openwrt") != NULL) {
				printf("Upgrade type: TEST Openwrt\n");
				strcpy(uip_httpd_file_name, "linux");
			} else if (strstr(start, "uImage") != NULL) {
				printf("Upgrade type: TEST uImage\n");
				strcpy(uip_httpd_file_name, "linux");
			} else if (strstr(start, "pcba") != NULL) {
				printf("Upgrade type: TEST PCBA\n");
				strcpy(uip_httpd_file_name, "pcba");
			} else if (strstr(start, "img") != NULL) {
				printf("Upgrade type: TEST ELF/IMG\n");
				strcpy(uip_httpd_file_name, "mpw");
			} else {
				printf("## Error: UNKNOWN IMAGE TYPE\n");
				webfailsafe_upload_failed = 1;
			}

			webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_TEST;
		}
		else
		{
