		...
		u32 desc0, u32 desc1, u32 desc2)
{
	...
	desc->flags = desc0;
	desc->cnt = desc1;
#ifdef CONFIG_SFAX8_DW_MMC
    desc->addr = virt_to_phys((const volatile void *)desc2);
    desc->next_addr = virt_to_phys((volatile const void *)((ulong)desc + sizeof(struct dwmci_idmac)));
#else
    desc->addr = desc2;
...
#ifdef CONFIG_SFAX8_DW_MMC
	dwmci_writel(host, DWMCI_DBADDR, (ulong)(virt_to_phys((volatile const void *)cur_idmac)));
#else
			...
			if (data->flags == MMC_DATA_READ) {
				if ((dwmci_readl(host, DWMCI_RINTSTS) &&
					...
					len = (len >> DWMCI_FIFO_SHIFT) &
						    DWMCI_FIFO_MASK;
#ifdef CONFIG_SFAX8_DW_MMC
					if(dwmci_readl(host, DWMCI_STATUS) & (1 << 3))
                        len = 0x200;
#endif
					for (i = 0; i < len; i++)
						*buf++ =
