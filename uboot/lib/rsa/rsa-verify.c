				   ...
				   uint sig_len, int node)
{
	...
	#ifdef CONFIG_SFAX8_SECURITY_BOOT
	/*TODO get rsa modulus(rsa pub key) from efuse, but now
	 Efuse can't work well, so we stored rsa modulus in blob.
	 When Efuse has a good work, the code should be changed to
	 collecting rsa modulus from Efuse.
	*/
