...
/// Descriptor status underba: Set by HW for TX DMA
#define DESC_STATUS_UNDERBA_BIT				CO_BIT(29)
/// Descriptor status ampdu: Set by HW for TX DMA
#define DESC_STATUS_AMPDU_BIT				CO_BIT(28)
/// Descriptor status whichdescriptor: Set by HW for TX DMA
#define DESC_STATUS_WHICHDESC_OFT			26
...
/// FEC Coding mask
#define FEC_CODING_PT_MASK      (0x1 << FEC_CODING_PT_OFT)
...
///Frame subtype
#define RX_HD_SUBTYPE 0xF0000000
///Frame type
#define RX_HD_FRAMETYPE 0x0C000000
...
struct tx_hd
{
	...
	uint32_t             first_pbd_ptr;   //c
	/// Data buffer associated with the header descriptor (for SW only).
	uint32_t             datastartptr;    //10
	/// MAC header buffer end (in the buffer associated with the header descriptor).
	uint32_t             dataendptr;	 //14
	/// Total length of the frame on air.
	uint32_t             frmlen;		//18
	/// MSDU lifetime parameter (for EDCA frame).
	uint32_t             frmlifetime;	//1c
	/// Valid only for A-MPDU header descriptor and for singleton MPDUs.
	uint32_t             phyctrlinfo;	//20
	/// Valid only for A-MPDU header descriptor and for singleton MPDUs.
	uint32_t             policyentryaddr; //24
	/// Optional length fields in case of BW drop
	uint32_t             optlen[3];		  //28 2c 30
	/// Valid only for A-MPDU header descriptor and for singleton MPDUs.
	uint32_t             macctrlinfo1;	 //34
	/// Valid only for MPDU header descriptor and for singleton MPDUs.
	uint32_t             macctrlinfo2;   //38
	/// Valid only for A-MPDU header descriptor and for singleton MPDUs.
	uint32_t             statinfo;		//3c
	/// Medium time used.
	uint32_t             mediumtimeused;  //40
};
...
#define TX_MIN_AMPDU_NB_PER_AC  3
/// Max number of AMPDU descriptors for the BK queue
#if (NX_TXDESC_CNT0 / 8) >= TX_MIN_AMPDU_NB_PER_AC
#define TX_MAX_AMPDU_NB_PER_AC0  (NX_TXDESC_CNT0 / 8)
#else
#define TX_MAX_AMPDU_NB_PER_AC0  TX_MIN_AMPDU_NB_PER_AC
#endif
/// Max number of AMPDU descriptors for the BE queue
#if (NX_TXDESC_CNT1 / 8) >= TX_MIN_AMPDU_NB_PER_AC
#define TX_MAX_AMPDU_NB_PER_AC1  (NX_TXDESC_CNT1 / 8)
#else
#define TX_MAX_AMPDU_NB_PER_AC1  TX_MIN_AMPDU_NB_PER_AC
#endif
/// Max number of AMPDU descriptors for the VI queue
#if (NX_TXDESC_CNT2 / 8) >= TX_MIN_AMPDU_NB_PER_AC
#define TX_MAX_AMPDU_NB_PER_AC2  (NX_TXDESC_CNT2 / 8)
#else
#define TX_MAX_AMPDU_NB_PER_AC2  TX_MIN_AMPDU_NB_PER_AC
#endif
/// Max number of AMPDU descriptors for the VO queue
#if (NX_TXDESC_CNT3 / 8) >= TX_MIN_AMPDU_NB_PER_AC
#define TX_MAX_AMPDU_NB_PER_AC3  (NX_TXDESC_CNT3 / 8)
#else
#define TX_MAX_AMPDU_NB_PER_AC3  TX_MIN_AMPDU_NB_PER_AC
#endif
#if (NX_BEACONING)
/// Max number of AMPDU descriptors for the BCN queue
#if (NX_TXDESC_CNT4 / 8) > TX_MIN_AMPDU_NB_PER_AC
#define TX_MAX_AMPDU_NB_PER_AC4  (NX_TXDESC_CNT4 / 8)
#else
...
extern struct tx_hw_desc       tx_hw_desc0[NX_TXDESC_CNT0];
/// Array of HW descriptors for BE queue
extern struct tx_hw_desc       tx_hw_desc1[NX_TXDESC_CNT1];
/// Array of HW descriptors for VI queue
extern struct tx_hw_desc       tx_hw_desc2[NX_TXDESC_CNT2];
/// Array of HW descriptors for VO queue
extern struct tx_hw_desc       tx_hw_desc3[NX_TXDESC_CNT3];
#if (NX_BEACONING)
