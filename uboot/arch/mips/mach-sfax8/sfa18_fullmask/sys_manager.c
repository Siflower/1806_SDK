...
*                                                                                                 *
***************************************************************************************************/
int sf_pad_set_func_2(u32 index)
{
	u8 tmp;
	int mod_cnt = 0;
	int mod_num = 0;
  ...
  tmp = readb((void *)FUNC_SW_IE(0xB9E30000,fun_cnt));
  tmp |= 1 << fun_num;
  writeb(tmp, (void *)FUNC_SW_IE(0xB9E30000,fun_cnt));

  tmp = readb((void *)FUNC_SW_SEL(0xB9E30000,fun_cnt));
  tmp |= (1 << fun_num);
  writeb(tmp, (void *)FUNC_SW_SEL(0xB9E30000,fun_cnt));

  tmp = readb((void *)PAD_FUCN_SEL(0xB9E30000,fun_cnt));
  tmp &= ~(1 << fun_num);
  writeb(tmp, (void *)PAD_FUCN_SEL(0xB9E30000,fun_cnt));

 // tmp = readb(PAD_MODE_SEL + mod_cnt * PAD_OFFSET);
  tmp = readb((void *)PAD_MODE_SEL(0xB9E30000,mod_cnt));
  tmp &= ~(0x3 << (mod_num * 2));
  tmp |= (0 << (mod_num * 2));
  // writeb(tmp, (PAD_MODE_SEL + mod_cnt * PAD_OFFSET));
  writeb(tmp, (void *)PAD_MODE_SEL(0xB9E30000,mod_cnt));
...
int sf_pad_set_func(u32 index)
{
	...
	fun_num = index % 8;
		...
		tmp = readb((void *)FUNC_SW_OEN(0xB9E30000,fun_cnt));
		tmp &= ~(1 << fun_num);
		writeb(tmp, (void *)FUNC_SW_OEN(0xB9E30000,fun_cnt));
		tmp = readb((void *)FUNC_SW_IE(0xB9E30000,fun_cnt));
		tmp &= ~(1 << fun_num);
		writeb(tmp, (void *)FUNC_SW_IE(0xB9E30000,fun_cnt));
		...
		writeb(0, (void *)GPIO_DIR(0xB9D00000, index));
