...
/*
* fix me: this is a workround solution, and will be removed when the RM#1545
* problem is solved.
*/
extern int npuphy_disable(void);
extern int usbphy_sleep(void);
extern void hold_all_module_reset(void);
...
static void linux_env_set(const char *env_name, const char *env_val)
{
	if (linux_env_idx < LINUX_MAX_ENVS - 1) {
		...
		if (CONFIG_IS_ENABLED(MALTA) || CONFIG_IS_ENABLED(SFA18)) {
			linux_env_p++;
			linux_env[++linux_env_idx] = linux_env_p;
		} else {
			*linux_env_p++ = '=';
