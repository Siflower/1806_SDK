...
void flush_cache(ulong start_addr, ulong size)
{
	...
	cache_loop(start_addr, start_addr + size, dlsize, HIT_WRITEBACK_SD);
}
...
void flush_dcache_range(ulong start_addr, ulong stop)
{
	...
	/* flush secondary-cache */
	cache_loop(start_addr, stop, lsize, HIT_WRITEBACK_SD);
}
...
#if 1
	cache_loop(start_addr, stop, lsize, HIT_WRITEBACK_INV_D);
	cache_loop(start_addr, stop, lsize, HIT_WRITEBACK_INV_SD);
#else
	/*invalidate secondary-cache*/
	cache_loop(start_addr, stop, lsize, HIT_INVALIDATE_SD);
